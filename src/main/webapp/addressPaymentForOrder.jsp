...
try
{
	...
	ResultSet rs1=st.executeQuery("select sum(total) from `db`.`cart` where email='"+email+"' and address is NULL");
	while(rs1.next())
	{
		total=rs1.getInt(1);
	}
%>
        ...
        <tbody>
 <%
      ResultSet rs= st.executeQuery("select * from `db`.`product` inner join `db`.`cart` on product.id=cart.product_id and cart.email='"+email+"' and cart.address is Null");
      while(rs.next())
      {
      %>        
          <tr>
...
<div class="right-div">
<h3>Pay online on this Tripathi@pay.com</h3>
